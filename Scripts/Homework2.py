...
def make_map(states, label):
    ...
    for state in states_abbrev.values():
        if state in skip:
            continue
        color = sm.to_rgba(states.ix[state])
        draw_state(ax, state, color=color,edgecolor='k')
...
#
# make_map(predictwise.Obama,"Probability Obama wins in state")
...
# winprob=100*sum(x>268 for x in result)/len(result)
# print("According to "+str(n_sim)+" simulations Obama's win percentage is" + str(winprob) +" %")

def plot_simulation(simulation):
    plt.hist(simulation, bins=np.arange(200, 538, 1),
             label='simulations', align='left', normed=True)
    plt.axvline(x=269,label="Victory Threshold",color="black")
    plt.axvline(x=332,label="Actual votes",color="red")
    spread=np.percentile(simulation,95)-np.percentile(simulation,5)

    pwin = ((simulation >= 269).mean() * 100)

    plt.title("Simulation result"+str(pwin)+"%  The spread is "+str(spread)+" votes.")
...
def gauss(x,mu,std):
    return 0.5*(1+erf((x-mu)/math.sqrt(2*std**2)))
...
# x=prediction_08['Dem_Adv'].as_matrix()
# y=prediction_08['Dem_Win'].as_matrix()


# plt.scatter(x,y,color="red")
# plt.plot(x,np.poly1d(np.polyfit(x,y,1))(x),"-",label="Regréssion linéaire")
# plt.xlabel("Prédiction Gallup")
# plt.ylabel("Résultats")
# plt.legend()
# plt.show()
#
# model = biased_gallup_poll(gallup_2012,8.068)
# model = model.join(electoral_votes)
# make_map(model.Obama, "P(Obama): Gallup + bias from 2008")
# plt.show()
# prediction = simulate_election(model, 10000)
# plot_simulation(prediction)
#print(prediction_08[(prediction_08.Dem_Adv > 0) & (prediction_08.Dem_Win < 0)].index.tolist())
...
# indices=e2008.Dem_Adv>0
# plt.scatter(e2008.pvi[indices],e2008.Dem_Adv[indices],color="red")
# plt.scatter(e2008.pvi[~indices],e2008.Dem_Adv[~indices],color="blue")
# plt.ylabel("Pvi in 2008 and 2012")
# plt.xlabel("Dem adv in 2008 and 2012")
# plt.axhline(y=0, color="black")
# plt.scatter(e2012.pvi,e2012.Dem_Adv,color="gray")
# plt.show()
...
def points_plot(e2008, e2012, clf):
    ...
    cs = ax.contourf(xx, yy, Z, cmap=cm.get_cmap('RdBu'), alpha=.5)
    cs2 = ax.contour(xx, yy, Z, cmap=cm.get_cmap('RdBu'), alpha=.5)
...
multipoll['poll_date'] = multipoll.start_date + (multipoll.end_date - multipoll.start_date) / 2
...
def default_missing(results):
    red_states = ["Alabama", "Alaska", "Arkansas", "Idaho","Louisiana", "Wyoming"]
...
def aggregated_poll_model(poll):
    sigma = avg.poll_std
    prob =  .5 * (1 + erf(avg.poll_mean / np.sqrt(2 * sigma**2)))
    return pd.DataFrame(dict(Obama=prob,Votes=avg.Votes), index=avg.index)
